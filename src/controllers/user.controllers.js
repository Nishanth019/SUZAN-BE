...
class UserController {
        ...
        try {
            ...
            if (existingUser && existingUser.isEmailVerified && existingUser.isUserVerified) {
                ...
                if (existingUser.isAdminVerified) {
                    ...
                    return res.status(400).json({ success: false, message: 'Your account has not yet been verified by our team. Once verified, we will reach out to you shortly.' });
                }
            }

            if (existingUser && existingUser.isEmailVerified && !existingUser.isUserVerified) {
                return res.status(201).json({ success: true, message: 'OTP is already Verified' });
            }
    ...
    completeStudentSignup = async (req, res) => {
        try {
            const { email, name, phone, gender, roll_no, program, branch, batch } = req.body;
    ...
    signin = async (req, res) => {
        try {
            ...
            if (!user.isEmailVerified) {
                return res.status(400).json({ success: false, message: 'Email not verified, Please complete the registration process' });
            }

            // Check if user's account is fully verified (for both admin and student)
            if (!user.isUserVerified) {
                return res.status(400).json({ success: false, message: 'Account not completely verified. Please complete the registration process' });
            }
    ...
    forgetPassword = async (req, res) => {
        try {
            ...
            await sendMail(email, 'Password Reset OTP', `Your password reset OTP is ${otp}`);
